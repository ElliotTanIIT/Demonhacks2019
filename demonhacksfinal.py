...
def run():
    ...
    for x in data_list[0]:
        ...
        old_time_format.append(x)
    data_list[0] = time_list_mins
    total = commute(data_list, profile)
    print("total fare: ${}".format(total))
    fare_w = tk.Toplevel(r)
    output_str = "First tap in at {}, transport: {}, at time: {}\n".format(data_list[1][0],data_list[2][0], old_time_format[0])
    for i in range(1,len(old_time_format)):
        output_str += "Next trip {} at {} by {} or within 2 hours of your first tap.\n".format(old_time_format[i], data_list[1][i], data_list[2][i])
    output_str += "Total Fare: ${}".format(total)
